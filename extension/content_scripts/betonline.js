...
// ── BetOnline content script ───────────────────────────────────
// Target page: betonline.ag/account/my-bets
//
// BetOnline renders bet history as an HTML table.
// Key selectors:
//   .bet-history-table              — outer table container
//   .bet-history-table tbody tr     — each bet row
//   td[data-label="Event"]          — event/game name
//   td[data-label="Type"]           — bet type
//   td[data-label="Selection"]      — pick
//   td[data-label="Odds"]           — odds
//   td[data-label="Stake"]          — stake
//   td[data-label="Date"]           — bet date
//   td[data-label="Status"]         — Won / Lost / Open
//
// Fallback: positional td indexing if data-label attrs absent.

(function () {
  const SPORTSBOOK = 'BetOnline';
  ...
  function scrapeBets() {
    ...
    rows.forEach(row => {
      try {
        ...
        console.debug('[BetTracker:BetOnline] row parse error:', err);
      }
    });
